...
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarColor02">
                <ul class="navbar-nav me-2 ms-auto">
                    ...
                    if (isset($_SESSION['id_cliente']) &&  isset($_SESSION['nome']) && isset($_SESSION['email'])) {
                        echo "
                            <li class='nav-item'>
                                <a class='bi bi-person-fill nav-link active' href='perfilc.php'>&nbsp; Perfil</a>
                            </li>
                        ";
    ...
    <div class="">
        ...
        $query = "SELECT id_cliente, cliente.nome AS nomecliente, estado.nome AS nomeestado, cidade.nome AS nomecidade, sexo, email, senha, telefone, cpf, rg, celular, rua, numero, complemento, cep, cliente.id_estado AS idestado, cliente.id_cidade AS idcidade FROM cliente INNER JOIN estado ON estado.id_estado = cliente.id_estado INNER JOIN cidade ON cidade.id_cidade = cliente.id_cidade WHERE id_cliente = $id";
        $exec = mysqli_query($conexao, $query);

        $cliente = mysqli_fetch_assoc($exec);
        //$query = "SELECT cliente.nome AS nomecliente, estado.nome AS nomeestado, cidade.nome AS nomecidade, cliente.id_estado AS idestado,  cliente.id_cidade AS idcidade, email, senha, id, cpf, rua, numero, cep, cliente.id_estado, cliente.id_cidade, complemento FROM cliente INNER JOIN estado ON estado.id_estado = cliente.id_estado INNER JOIN cidade ON cidade.id_cidade = cliente.id_cidade WHERE id = $id_cliente ";
        //$executar = mysqli_query($conexao,$query);
        //$cliente = mysqli_fetch_array($executar);
        ...
        <div>
            ...
            <div class="m-4 jumbotron">
                <h1 class="display-4"><i class="fas fa-address-card"></i> Editar cliente</h1>
                <hr class="my-4">
            </div>

            <form action="atualizar-usuario-processa.php" method="POST" class="container">

                <div class="mb-3">
                    <h5 class="form-label"><i class="fas fa-dice"></i>&nbsp; Dados pessoais</h5>
                    <input class="form-control" type="text" name="nome" maxlength="" placeholder="Digite seu nome" required="required" value="<?php echo $cliente['nomecliente']; ?>"><br>
                    <input class=" form-control" type="text" name="rg" placeholder="Digite o RG" required="required" value="<?php echo $cliente['rg']; ?>"><br>
                    <input class="form-control" type="text" name="cpf" placeholder="Digite o CPF" required="required" value="<?php echo $cliente['cpf']; ?>">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" type="email" name="email" placeholder="Digite seu e-mail" required="required" value="<?php echo $cliente['email']; ?>"><br>
                    <input class="form-control" type="password" name="senha" placeholder="Digite sua senha" required="required" value="<?php echo $cliente['senha']; ?>">
                </div>
                <div class="mb-3">
                    <h5 class="form-label"><i class="fas fa-phone-square-alt"></i>&nbsp; Telefones</h5>
                    <input class="form-control" type="text" name="telefone" placeholder="Digite o telefone residencial" value="<?php echo $cliente['telefone']; ?>"><br>
                    <input class="form-control" type="text" name="celular" placeholder="Digite o telefone celular" value="<?php echo $cliente['celular']; ?>">
                </div>
                <div class="mb-3">
                    <h5 class="form-label"><i class="fas fa-map-marked-alt"></i>&nbsp; Endereço</h5>
                    <input class="form-control" type="text" name="rua" placeholder="Digite o nome da rua" required="required" value="<?php echo $cliente['rua']; ?>"><br>
                    <input class="form-control" type="text" name="numero" placeholder="Digite o número da rua" required="required" value="<?php echo $cliente['numero']; ?>"><br>
                    ...
                    <!-- <i class="bi bi-info-square-fill flex-shrink-0 align-items-center me-1 alert alert-warning" role="alert">&nbsp; Complemento opcional.</i>
                             -->
                    <div class="d-flex">
                        <textarea class=" me-2 form-control" data-bs-toggle="collapse" data-bs-target="#collapseExample" name="complemento" placeholder="Digite o complemento"><?php echo $cliente['complemento']; ?></textarea>
                        <div class="collapse" id="collapseExample">
                            <div class="card card-body alert-success ">Complemento opcional.</div>
                        </div>
                    </div><br>
                    <input class="form-control" type="text" name="cep" placeholder="Digite o CEP." required="required" value="<?php echo $cliente['cep']; ?>"><br>
                    ...
                    <select class="form-select" name="estados" id="estados" required="required">
                        ...
                        //}
                        echo "<option value='$cliente[idestado]'>$estado1</option>";
                        ...
                        <option value='$cliente[idcidade]'>$city1</option>
                        ";
