...
<head>
    ...
    <head>
        ...
        <title>Cadastrar costureira</title>
    </head>
                    ...
                    if (empty($_SESSION['nome']) && empty($_SESSION['email'])) {
                        ...
                        <li class='nav-item dropdown'>
                            ...
                            <div class='dropdown-menu dropdown-menu-end success'>
                                ...
                                <a class='dropdown-item active' href='cadastrar-costureira.php'><i class='fas fa-cut'></i> Cadastrar costureira</a>
    ...
    <div class="">
        ...
        <div>
            ...
            <div class="m-4 jumbotron">
                <h1 class="display-4"><i class="fas fa-address-card"></i> Cadastrar costureira</h1>
                <hr class="my-4">
            </div>

            <form action="cadastrar-costureira-process.php" method="POST" class="container">
                ...
                <div class="mb-3">
                    ...
                    <div>
                        <select id="cidades" class="form-select" style="display: none;" name="cidades"></select>
                    </div>
                </div>


                <div class="mb-3">
                    <h5 class="form-label"><i class="fas fa-tshirt"></i>&nbsp; Tipos de serviços</h5>
                    ...
                    while ($roupa = mysqli_fetch_array($executas)) {
                        ...
                        $consultats = "SELECT id_tiposervico, foto, tipo_servico.nome AS tiponome, valor, descricao, tipo_servico.id_tiporoupa, tipo_roupa.id_tiporoupa, tipo_roupa.nome FROM tipo_servico INNER JOIN tipo_roupa ON tipo_roupa.id_tiporoupa = tipo_servico.id_tiporoupa WHERE tipo_servico.id_tiporoupa =$roupa[id_tiporoupa]  ";
                        $executar = mysqli_query($conexao, $consultats);
                        //print_r($consulta); exit;

                        $roup = utf8_encode($roupa['nome']);
                        echo "
                            <div class='mb-3'>
                            <label for='exampleFormControlInput1' class='form-label'>$roup:</label>";

                        // <label for="exampleFormControlInput1" class="form-label">Sexo:</label>
                        // <div class="form-check mb-3">
                        // <input class=" form-check-input form-check-inline" type="checkbox" value="F" name="sexo" required="required"><i class="fas fa-venus"></i>&nbsp;Feminino</input><br>
                        // <input class="form-check-input form-check-inline" type="checkbox" value="M" name="sexo" required="required"><i class="fas fa-mars"></i>&nbsp;Masculino </input><br>
                        // <input class="form-check-input form-check-inline" type="checkbox" value="I" name="sexo" required="required"><i class="fas fa-genderless"></i>&nbsp;Prefere não dizer </input><br>
                        // </div>

                        $a = 0;

                        while ($ts = mysqli_fetch_array($executar)) {
                            $prod = utf8_encode($ts['tiponome']);
                            if ($a == 3) {
                                $a = 0;
                            }
                            $a = $a + 1;
                            echo "  
                                    <div class='form-check'>
                                            <input class='form-check-input form-chek-inline' type='checkbox' name='tserv[]' value='$ts[id_tiposervico]'> $prod
                ...
                <div class="mb-3 btn-group">
                    ...
                    <button id="button" type="submit" class="bi bi-check-circle-fill rounded btn btn-outline-success"> Cadastrar</button>
                </div>
            </form>
        </div>
    </div>
